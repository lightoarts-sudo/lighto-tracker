...
old_dict = content[start_idx:end_idx]
...
for i, (rank, r) in enumerate(qualified, 1):
    ...
    name = f"auto_top{i}_4h_d{delay_bars}_r{max_rank}_chg{min_change}-{max_change}_green_uw{max_upper_wick:.1f}_reclaim_sl{sl_pct}_be{be_after}_tr{trail_start}x{trail_giveback}_t{time_stop}"
...
# Find the position to insert - after the last auto strategy comment
# Look for the comment "# === Auto-optimized strategies from 4H scheduler"
...
# Find the end of the existing auto strategies (before the closing brace of the dict)
# We need to replace from the comment to just before the closing brace of the dict
# Find the next strategy after auto_top3 or the closing brace
search_start = comment_idx
while search_start < end_idx and content[search_start] != '}':
    search_start += 1
# Actually, let's find the line with the last auto strategy
# The existing auto strategies end before the closing brace of the dict

# Better approach: Replace the entire block from the comment to the closing brace of the dict
# but we need to keep the rest of the dict (everything before the comment)

# Find the start of the dict content (after "MICRO_TOP10_OPTIMIZED_STRATEGIES = {")
dict_content_start = content.find("{", start_idx) + 1

# Get everything before the auto strategies comment
...
# ============================================================
# 2. Update microActiveStrategies in CONFIG
# ============================================================

# Find the microActiveStrategies line
# It's in the CONFIG dict as a _csv_env call
