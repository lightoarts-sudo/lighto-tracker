...
# Find the start - handle both formats
start_marker1 = "MICRO_TOP10_OPTIMIZED_STRATEGIES = {"
start_marker2 = "MICRO_TOP10_OPTIMIZED_STRATEGIES = \n    \""
...
# Find the closing brace by counting braces from the first {
...
brace_count = 0
end_idx = first_brace
for i, ch in enumerate(content[first_brace:], first_brace):
    if ch == '{':
        brace_count += 1
    elif ch == '}':
        brace_count -= 1
...
if comment_idx == -1:
    # If comment not found, find the last strategy before the closing brace
    # We'll insert before the last }
    comment_idx = end_idx - 1
    while comment_idx > first_brace and content[comment_idx] in ' \n\r\t':
        comment_idx -= 1
    # Find the start of the last strategy
    # Actually let's just use end_idx - 1 (before closing brace)
    comment_idx = end_idx - 1

# Get everything before the auto strategies comment (from dict start)
dict_content_start = first_brace + 1
...
# Replace the entire dict (from start_idx to end_idx)
